...
export function OyuncuKarti({
  ...
  // Oy geldiğinde shake efekti
  useEffect(() => {
    if (voteCount > prevVoteCount) {
      setJustVoted(true)
      const timeout = setTimeout(() => setJustVoted(false), 600)
  ...
  return (
    <button
      ...
      className={`
        relative w-full text-left rounded-lg p-3 transition-all duration-300
        ${player.isAlive ? "player-card-alive" : "player-card-dead opacity-50"}
        ${selectable && !isMe && player.isAlive ? "hover:bg-gray-100 dark:hover:bg-gray-700/50 cursor-pointer" : "cursor-default"}
        ${isSelected ? "player-card-selected ring-2 ring-red-500 bg-red-50 dark:bg-red-500/10" : "bg-white/80 dark:bg-gray-800/50 shadow-sm"}
        ${isMe && player.isAlive ? "border border-blue-500/30 animate-breathe" : "border border-gray-100 dark:border-transparent"}
        ${isVotedByMe ? "ring-2 ring-yellow-500 bg-yellow-50 dark:bg-yellow-500/10" : ""}
        ${justVoted ? "animate-shake" : ""}
      `}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <div
              className={`w-11 h-11 rounded-full flex items-center justify-center text-lg overflow-hidden transition-all duration-500 border-2 ${player.isAlive
            ...
            >
              {player.role ? (
                ...
                <img
                  ...
                  alt={player.role}
                  className={`w-full h-full object-cover transition-transform duration-500 ${player.isAlive ? "hover:scale-110" : ""}`}
                />
              ) : player.isAlive ? (
            ...
            {/* Baglanti durumu */}
            <div
              className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-gray-800 z-10 transition-all duration-300 ${player.isConnected ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" : "bg-gray-500"
                }`}
            />
          </div>
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className={`font-medium transition-all duration-300 ${player.isAlive ? "text-gray-900 dark:text-white" : "text-gray-500 line-through"}`}>
                {player.username}
              </span>
              {isMe && (
            ...
            {roleInfo && (
              <span className={`text-xs transition-colors duration-300 ${roleInfo.team === "mafia" ? "text-red-500 dark:text-red-400" : "text-blue-600 dark:text-blue-400"}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          {phase === "day_voting" && voteCount > 0 && (
            <Badge variant="destructive" className="text-xs vote-count-animate" key={`vote-${voteCount}`}>
          ...
          {isSelected && (
            <Badge variant="destructive" className="animate-pulse">
              <Target className="h-3 w-3 mr-1 animate-spin-slow" />
              Hedef
            </Badge>
          )}
